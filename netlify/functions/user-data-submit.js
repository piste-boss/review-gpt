...
export const handler = async (event, context) => {
  ...
  if (!submitGasUrl) {
    return jsonResponse(400, { message: 'ユーザー情報保存GASエンドポイントが設定されていません。' })
  }

  if (!spreadsheetId) {
    return jsonResponse(400, { message: 'ユーザー情報のスプレッドシートIDが取得できませんでした。' })
  }
  ...
  try {
    ...
    return jsonResponse(502, { message: 'ユーザー情報の送信中にエラーが発生しました。' })
  }
}
